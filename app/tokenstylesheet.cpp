...
/* The spin buttons and the combo drop-down are DELIBERATELY UNSTYLED.
   ...
   STILL OUTSTANDING: the SPIN arrows do not come back, because styling
   QSpinBox at all is enough to switch it to QStyleSheetStyle, and that draws
   the subcontrol from the sheet whether or not the sheet mentions it. The two
   ways out are to leave QSpinBox entirely unstyled -- which makes it the one
   input that does not match the others -- or to paint the arrows. The second
   is the right answer and it wants doing alongside the tab close button, which
   needs its own painted widget for the same reason. Both are chrome the
   redesign already calls for; neither belongs in a stylesheet. */
