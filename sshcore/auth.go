...
func keyboardInteractive(cfg *Config) ssh.KeyboardInteractiveChallenge {
	return func(user, instruction string, questions []string, echos []bool) ([]string, error) {
		...
		for i, q := range questions {
			// A pre-supplied answer wins over everything, including the
			// password auto-answer: a caller that went and asked its user
			// this exact question has better information than a heuristic
			// on the word "password".
			if a, ok := cfg.KeyboardAnswers[q]; ok {
				answers[i] = a
				continue
			}
			...
			if cfg.AuthPrompt == nil {
				// Not "no handler" -- that describes our plumbing, and the
				// caller can do nothing with it. This says WHAT was asked and
				// whether the answer is a secret, so a UI can put the question
				// in front of a person and dial again with the answer. The
				// marker is matched by that UI; see KeyboardPromptMarker.
				return nil, fmt.Errorf("%s %s: %s", KeyboardPromptMarker,
					echoTag(echos, i), q)
			}
